...
def _git_changed_files(from_rev: str | None = None, to_rev: str | None = None) -> tuple[str, str | None, list[str]]:
    ...
    return from_rev, to_rev, [line.strip() for line in output.splitlines() if line.strip()]
...
def git_report(from_rev: str | None = None, to_rev: str | None = None, include_files: Callable[[str], bool] = lambda x: True) -> str:
...
def _main(args: list[str] | None = None):
    ...
    parser.add_argument("--files", "-f", type=str, default="*.py", help="The files to compare (default: *.py)")
